locals {
  ...
  lambda_common_libs_layer_path = "${path.module}/layers/${var.lambda_layer}"
  lambda_common_libs_layer_zip_name = "${local.build_directory_path}/${var.lambda_layer}.zip"
}

resource "null_resource" "common_nodejs_layer" {
...
data "archive_file" "common_libs_layer_package" {
  type = "zip"
  source_dir = local.lambda_common_libs_layer_path
  output_path = local.lambda_common_libs_layer_zip_name

  depends_on = [null_resource.common_nodejs_layer]
}

resource "aws_lambda_layer_version" "common_nodejs_layer" {
  layer_name = var.lambda_layer
  filename = local.lambda_common_libs_layer_zip_name
  source_code_hash = data.archive_file.common_libs_layer_package.output_base64sha256
